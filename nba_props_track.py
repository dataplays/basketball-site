"""
NBA Props — forward performance tracker.
...
Mirrors wnba_props_track.py (same logic; NBA file-name prefix + ESPN feed
come from nba_props_grade).

Usage:
    py -3 nba_props_track.py                # catch up all settled days not yet tracked
    py -3 nba_props_track.py --date 2026-04-15   # (re)grade one specific day
    py -3 nba_props_track.py --rebuild      # wipe and rebuild from all PDFs
    py -3 nba_props_track.py --summary      # just print the cumulative summary
    py -3 nba_props_track.py --dedupe       # collapse any duplicate rows (safety)
"""
...
TRACKER = DOCS / "nba_props_tracker.csv"
...
def pdf_for(ymd: str) -> Path:
    ...
    return DOCS / f"NBA_Props_Projections_{INV_MON[ymd[4:6]]}{ymd[6:8]}_2026.pdf"
...
def print_summary(since: str | None = None) -> None:
    ...
    if since:
        ...
        if not rows:
            ...
            return
        _summary_block(rows, f"NBA PROPS — SINCE {since[:4]}-{since[4:6]}-{since[6:8]}")
    else:
        _summary_block(allrows, "NBA PROPS — CUMULATIVE (ALL MODELS)")
        new_rows = [r for r in allrows if r["date"] >= NEW_MODEL_SINCE]
        if new_rows:
            print()
            _summary_block(new_rows, "NBA PROPS — NEW MODEL ONLY  (v2026-07-01, Jul 1 2026+)")
...
def main():
    ap = argparse.ArgumentParser(description="NBA props forward tracker")
