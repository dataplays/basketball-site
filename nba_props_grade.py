"""
NBA Props Projections — Win/Loss back-test.

Grades the TOP 10 picks (by EV%) from each day's
NBA_Props_Projections_<MonDD>_2026.pdf against actual ESPN box scores.
...
- Real over/under odds are read from the PDF's "All-Book Lines"
  (line-shopping) section at the recommended book where available;
  days/picks with no stored multi-book price fall back to -110 (flagged *).
- Actual points / rebounds come from the ESPN NBA summary boxscore.

Output: per-day W-L-Push + net units, and a season total.

Mirrors wnba_props_grade.py; the grading/PDF/box-score logic is league-agnostic
(same PDF format and ESPN boxscore schema), so only the league endpoints,
file-name prefix, and labels differ.
"""
...
ESPN_SB = "https://site.web.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard"
ESPN_SUM = "https://site.web.api.espn.com/apis/site/v2/sports/basketball/nba/summary"
...
def run_config(top_n: int | None = 10, min_ev: float | None = None):
    ...
    files = sorted(
        (f for f in glob.glob(str(DOCS / "NBA_Props_Projections_*_2026.pdf"))
...
def sweep():
    ...
    print()
    print("=" * 94)
    print("  NBA PROPS — THRESHOLD SWEEP — net units at actual odds (real) / -110 proxy (full)")
    print("=" * 94)
    print(f"  {'Universe':14} | {'FULL (incl. -110 proxy days)':^34} | {'REAL-ODDS (stored book prices)':^34}")
    ...
    print("  " + "-" * 90)
    print("  Ranked by real-odds ROI (the trustworthy signal). Break-even at ~ -110 juice = 52.4% win.")
    print("=" * 94)
...
def main(min_ev: float | None = None, top_n: int | None = None):
    ...
    print()
    print("=" * 72)
    print(f"  NBA PROPS — {universe} — WIN/LOSS BACK-TEST")
    print("=" * 72)
    ...
    print()
    print("  -- Real-odds only (actual stored recommended-book prices) --")
    ...
    print()
    print("  * -110 = days with no stored multi-book odds; units use the -110 proxy.")
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="NBA props win/loss back-test")
