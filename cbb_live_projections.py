"""
Live College Basketball Projections Dashboard

Flask web app that shows live NCAA basketball games with current scores,
...
Usage:
    py -3 cbb_live_projections.py              # today's games, port 5000
    py -3 cbb_live_projections.py --port 8080  # custom port
    py -3 cbb_live_projections.py --date 2026-03-01  # specific date
"""
...
REGULATION_MIN = 40.0   # 2 × 20-minute halves
HALF_MIN = 20.0
OT_MIN = 5.0
HALF_POSS_SHARE = 0.4725  # ~47.25% of total possessions in 1H
...
# scoring regresses toward league-average PPP. Tuned for men's CBB scoring
# (~72 pts/team, 40-min game): onset earlier than the NBA but capped lower.
BLOWOUT_THRESHOLD = 13   # point lead where regression kicks in
...
def fetch_torvik_ratings() -> tuple[dict, dict, str]:
    """Fetch adjusted OE/DE from Bart Torvik. Returns (oe_data, de_data, source_label)."""
    import requests as req_lib
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    r = req_lib.get(TORVIK_URL, headers=headers, timeout=20)
    r.raise_for_status()
    ...
    oe_data = {name: (val, rank) for rank, (name, val) in enumerate(oe_list, 1)}

    de_list.sort(key=lambda x: x[1])
    de_data = {name: (val, rank) for rank, (name, val) in enumerate(de_list, 1)}

    # ESPN aliases
    for wn_name, espn_name in [("FAU", "Florida Atlantic"), ("Loyola-Chicago", "Loyola Chicago"),
                                ("Loyola-Maryland", "Loyola Maryland"), ("UMass-Lowell", "UMass Lowell")]:
...
def load_all_ratings() -> None:
    ...
    source = "WarrenNolan"
    try:
        print("Scraping OE from WarrenNolan...")
        oe = scrape_wn_ratings(WN_OE_URL)
        print(f"  Got {len(oe)} teams.")
        print("Scraping DE from WarrenNolan...")
        de = scrape_wn_ratings(WN_DE_URL)
        print(f"  Got {len(de)} teams.")
    except (URLError, OSError, Exception) as e:
        print(f"  WarrenNolan unavailable: {e}")
        print("  Falling back to Bart Torvik...")
        oe, de, source = fetch_torvik_ratings()
        print(f"  Got {len(oe)} OE and {len(de)} DE from Torvik.")
...
def fetch_live_scoreboard(date_str: str) -> list[dict]:
    """
    Fetch all D1 games with live scores from ESPN API.
    ...
    for event in data.get("events", []):
        ...
        game = {
            ...
            "away_1h_score": _get_linescore(away, 0),
            "home_1h_score": _get_linescore(home, 0),
...
def fetch_game_fouls(game_id: str) -> dict:
    """
    Fetch per-half team fouls from ESPN summary play-by-play.
    Returns {"away_1h": N, "away_2h": N, "home_1h": N, "home_2h": N,
    ...
    empty = {"away_1h": 0, "away_2h": 0, "home_1h": 0, "home_2h": 0,
    ...
    try:
        rosters = raw.get("rosters") or raw.get("boxscore", {}).get("players", [])
    ...
    counts = {"away_1h": 0, "away_2h": 0, "home_1h": 0, "home_2h": 0}
    plays = raw.get("plays", [])
    for play in plays:
        ...
        if period == 1:
            ...
            counts[f"{side}_2h"] += 1  # 2H + any OT fouls go in 2H bucket

    counts["away_total"] = counts["away_1h"] + counts["away_2h"]
    counts["home_total"] = counts["home_1h"] + counts["home_2h"]
    ...
    with ThreadPoolExecutor(max_workers=min(len(live_ids), 8)) as pool:
        ...
        for fut in futures:
            ...
            try:
                ...
                results[gid] = {"away_1h": 0, "away_2h": 0, "home_1h": 0,
                                "home_2h": 0, "away_total": 0, "home_total": 0}
    return results
...
def project_game(game: dict) -> dict:
    ...
    if state == "pre":
        ...
        if period <= 2:
            ...
            total_game_min = REGULATION_MIN + OT_MIN * (period - 2)
        ...
        if "half" in detail and clock_min < 0.1:
            # Halftime
            time_elapsed = HALF_MIN
            total_game_min = REGULATION_MIN
        elif period == 1:
            time_elapsed = HALF_MIN - clock_min
            total_game_min = REGULATION_MIN
        elif period == 2:
            time_elapsed = REGULATION_MIN - clock_min
            total_game_min = REGULATION_MIN
        else:
            # Overtime
            ot_num = period - 2
    ...
    if state == "pre":
    ...
    elif period == 1 and state == "in":
        # Currently in 1H: project remaining 1H possessions
        elapsed_1h_min = HALF_MIN - (clock_sec / 60.0)
        ...
        # 2H, halftime, OT, or post: use actual 1H scores if available
    ...
    h1_is_actual = (state != "pre" and period >= 2 and
...
def compute_live_pace_stats(g: dict) -> dict:
    ...
    and each team's current points-per-possession hold, project the final
    score/total/margin — and, while the first half is still in progress,
    the 1H final score/total/margin.
    """
    out = {
    ...
    if g.get("period", 0) <= 1 and elapsed < HALF_MIN:
...
<title>CBB Live Projections</title>
<style>
  :root {
    --bg: #0f1923;
    --card-bg: #1a2634;
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .proj-stat.box-pace { border: 1px dashed rgba(255,68,68,0.45); }
...
<header>
  <h1><span>&#9679;</span> CBB Live Projections</h1>
    ...
    <div class="game-card">
      ...
      <div class="fouls-row">
        <div class="foul-team">
          ...
          <span class="foul-half">1H: <span class="num">{{ g.away_1h }}</span></span>
          <span class="foul-half">2H: <span class="num">{{ g.away_2h }}</span></span>
          {% set away_current_half = g.away_1h if g.period == 1 else g.away_2h %}
          {% if away_current_half >= 10 %}<span class="dbl-bonus">DBL BONUS</span>
          {% elif away_current_half >= 7 %}<span class="bonus">BONUS</span>{% endif %}
        </div>
        <span class="foul-center">Team Fouls</span>
        <div class="foul-team">
          {% set home_current_half = g.home_1h if g.period == 1 else g.home_2h %}
          {% if home_current_half >= 10 %}<span class="dbl-bonus">DBL BONUS</span>
          {% elif home_current_half >= 7 %}<span class="bonus">BONUS</span>{% endif %}
          <span class="foul-half">1H: <span class="num">{{ g.home_1h }}</span></span>
          <span class="foul-half">2H: <span class="num">{{ g.home_2h }}</span></span>
...
def fetch_and_project() -> tuple[list[dict], list[dict], list[dict], str, str | None]:
    ...
    for g in projected:
        ...
        if fouls:
            ...
            g.update({"away_1h": 0, "away_2h": 0, "home_1h": 0, "home_2h": 0,
                       "away_total": 0, "home_total": 0})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CBB Live Projections Dashboard")
    ...
    print("=" * 50)
    print("  CBB Live Projections Dashboard")
    print("=" * 50)
