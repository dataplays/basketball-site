...
Scrapes all needed data and projects points & rebounds for every player
in today's NBA games, using the framework from the NBA Props paper:
  1. Weighted rolling minutes (last 15 games, recent-weighted)
  2. Injury-driven minute redistribution
  3. B2B / blowout / foul adjustments
  4. Per-minute production rates (PPM, RPM)
  5. Opponent defensive rating & rebounding adjustments
  6. Pace adjustment
  7. Correlated bootstrap Monte Carlo: points and rebounds are drawn from
     the SAME historical game (recency- and minute-weighted) and replayed
     scaled to projected minutes, preserving the within-game pts<->reb
     correlation the points+rebounds combo prop depends on.
  8. Model win-probabilities are de-vigged and blended with the no-vig
     market price before EV, trimming false edges on thin samples.

Usage:
    py -3 nba_props_projections.py                  # today's games
    py -3 nba_props_projections.py --date 2026-03-17  # specific date
    py -3 nba_props_projections.py --pdf             # also generate PDF
"""
...
ESPN_ROSTER = (
    "https://site.web.api.espn.com/apis/site/v2/sports/basketball/nba/teams/{team_id}/roster"
)
ESPN_GAMELOG = (
    "https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba/"
    "athletes/{player_id}/gamelog?season=2026&seasontype=2"
)
ESPN_PROP_BETS = (
...
THE_ODDS_API_BASE = "https://api.the-odds-api.com/v4/sports/basketball_nba"
...
# League averages (approximate 2025-26 season)
LEAGUE_AVG_DRTG = 115.5
LEAGUE_AVG_PACE = 99.2
LEAGUE_AVG_REB_PG = 43.5
...
# WNBA tracker evidence (593 graded picks): every stated-EV bucket below 25
# lost money and absolute EV had ~zero correlation with winning — small stated
# edges are almost entirely model noise + hold, so they no longer produce picks.
MIN_REC_EV = 5.0
MIN_REC_BOOKS = 3
MIN_REC_PROB_DELTA = 0.04

# Home-court advantage on scoring efficiency: NBA HCA ~3.0 pts (±1.5 per side)
# over an ~115-pt team score ≈ ±1.3%. 0 at neutral sites.
HCA_PCT = 0.013
...
# unshrunk; a 2-3 game sample regresses hard toward league average so it
# can't post extreme ppm/rpm that turn into phantom edges.
RATE_SHRINK_FULL_WMIN = 240.0
LEAGUE_AVG_PPM = 0.48   # ~115 team pts / 240 team player-minutes
LEAGUE_AVG_RPM = 0.18   # ~44 team reb / 240 team player-minutes

# ESPN's injury feed rarely says doubtful/questionable — day-to-day is the
# common game-time-decision status. Those players stay in the projections
# (they usually play) but are flagged "gtd" in the output, and their potential
# absence redistributes to teammates at partial strength.
...
def fetch_roster(team_id: str) -> list[dict]:
    """Fetch a team's roster. Returns list of {id, name, position}."""
...
def fetch_player_gamelog(player_id: str) -> list[dict]:
    """
    Fetch a player's 2025-26 regular season game log from ESPN.
    Returns list of dicts with min, pts, reb, fg, fga, fg3, fg3a, ft, fta, pf
    ordered most recent first.
    Labels: MIN, FG, FG%, 3PT, 3P%, FT, FT%, REB, AST, BLK, STL, PF, TO, PTS
    """
    url = ESPN_GAMELOG.format(player_id=player_id)
    data = fetch_json(url)
    if not data:
        return []
    ...
    # Collect all events across months (most recent month first)
    games = []
        ...
        for cat in st.get("categories", []):
            for evt in cat.get("events", []):
                ...
                if not stats:
                    continue
                try:
                    mins_raw = stats[idx.get("MIN", 0)]
                    pts_raw = stats[idx.get("PTS", 13)]
                    reb_raw = stats[idx.get("REB", 7)]
                    pf_raw = stats[idx.get("PF", 11)]
                    fg_raw = stats[idx.get("FG", 1)]      # "3-13"
                    ft_raw = stats[idx.get("FT", 5)]       # "11-16"
                    tpt_raw = stats[idx.get("3PT", 3)]     # "1-2"

                    mins = int(mins_raw) if mins_raw and mins_raw != "--" else 0
                    pts = int(pts_raw) if pts_raw and pts_raw != "--" else 0
                    reb = int(reb_raw) if reb_raw and reb_raw != "--" else 0
                    pf = int(pf_raw) if pf_raw and pf_raw != "--" else 0
    ...
    # ESPN returns months in reverse chronological order, events within each
    # month are also most-recent-first — so `games` is already most-recent-first
    return games
...
def fetch_prop_lines(event_id: str) -> dict:
    ...
    Returns {player_id: {
        "pts_line": float, "pts_over_odds": int, "pts_under_odds": int,
        "reb_line": float, "reb_over_odds": int, "reb_under_odds": int,
        "pr_line": float, "pr_over_odds": int, "pr_under_odds": int,
    }}
    ESPN returns props in Over/Under pairs (Over first, Under second).
    """
    ...
    props = {}  # player_id -> dict

    # Process items in pairs: items arrive as Over, Under, Over, Under, ...
    i = 0
    while i < len(items) - 1:
        item1 = items[i]
        item2 = items[i + 1]

        # Check if these two are a pair (same athlete, same type, same total)
        ...
        if ref1 == ref2 and type1 == type2 and total1 == total2:
            # Skip if total value is empty or missing
            if not total1:
                i += 2
                continue
...
def _normalize_team(name: str) -> str:
    """Normalize team display names (e.g. 'Boston Celtics') for matching."""
...
def fetch_odds_api_props(
    ...
    """
    # 1. Get all NBA events on The Odds API
...
# WNBA tracker showed picks claiming an average ~70% win probability realized
# 50.6% over 579 graded picks, and solving the post-fix rows for the weight
# that matches reality gives w ≈ 0.24. The market is the sharper estimator;
# the model's job is to find the rare big disagreements.
...
NUM_SIMS = 10000  # number of simulation iterations per player
...
def simulate_player(
    ...
    pace_adj: float,
    usage_boost: float,
    max_min_cap: float = 44.0,
    hca_mult: float = 1.0,
) -> dict:
    """
    Run a correlated bootstrap Monte Carlo for a player's points and rebounds.
    ...
    the points+rebounds combo prop depends on, and ties efficiency to the
    minutes actually played in that game.

    Returns dict with sim_pts (list), sim_reb (list), sim_pr (list),
    ...
    if n_games < 3 or expected_min < 1:
        # Not enough data — fall back to deterministic projection (rounded to int)
    ...
    # Minutes variability: recency-weighted std of the same window, so an old
    # role (e.g. bench minutes before a promotion) doesn't inflate the minute
    # noise around the player's current role.
    ...
    if w_avg_min > 0:
        min_std = max(
            ...
            1.5,  # floor: at least 1.5 min std dev
        )
    else:
        min_std = 3.0
    ...
    for g in drawn:
        # 1. Simulate minutes: normal draw centered on expected_min
        #    with the player's actual variability
        sim_min = random.gauss(expected_min, min_std)
        sim_min = max(0.0, min(sim_min, max_min_cap))  # hard cap: player's ceiling

        # 2. Replay the drawn game scaled to tonight's projected minutes. This
        #    ties efficiency to minutes — a low-minute game's line is no longer
        #    stretched across a full projected workload as if the rate were
        #    stable — and keeps pts/reb from the same game (correlated).
        scale = sim_min / g["min"]

        # Apply today's matchup adjustments (same factors as deterministic)
...
def compute_player_stats(games: list[dict]) -> dict | None:
    ...
    Returns dict with weighted_mpg, ppm, rpm, avg_pf, ftr, fg3_rate, etc.
    """
    if not games:
        return None

    # Take up to 15 most recent games
    recent = games[:15]
    n = len(recent)
    ...
    w_sum = sum(weights)
    total_w_min = w_min  # weighted total minutes
    ...
    # Average personal fouls per game (weighted)
    w_pf = sum(w * g["pf"] for w, g in zip(weights, recent))
    avg_pf = w_pf / w_sum

    # Free throw rate (FTA / FGA) — season-wide from available games
    ...
    # Season averages (for reference / display)
    all_games = games  # full season
    ...
    # Max minutes in any of the last 40 games played — used as a hard cap on
    # projected minutes so we never project a player above their observed ceiling.
    last_40 = games[:40]
...
def fetch_team_schedule_b2b(team_id: str, target_date: str) -> bool:
    """
    Check if a team is on the 2nd night of a back-to-back by looking at
    yesterday's scoreboard for that team.
    """
...
def project_player(
    ...
    # Injury redistribution: estimate extra minutes from absent teammates
    ...
    for tm in injured_teammates:
        ...
        # Positional redistribution: same-position gets most, others get some
    ...
    # Hard cap: never project above the player's max minutes in any of their
    # last 40 games. Falls back to 42.0 if no max is available.
    max_min_cap = stats.get("max_min_last_40") or 42.0
    expected_min = min(expected_min, max_min_cap)

    # ── B. Expected Points ──

    ppm = stats["ppm"]

    # Usage boost from injured teammates (star absent -> more usage)
    usage_boost = 0.0
    for tm in injured_teammates:
        tm_stats = tm.get("stats")
        if not tm_stats:
    ...
    # Opponent rebounding adjustment: use opponent OE as proxy
    # (higher OE = better shooting = fewer misses = fewer defensive rebounds for us)
    # We use inverse: poor-shooting opponents (low OE) give more rebound opportunities
    opp_oe = opp_ratings.get("oe", LEAGUE_AVG_DRTG)
    # Opponent missed shots proxy: lower OE = more misses = more reb opportunities
    reb_opp_adj = (2 * LEAGUE_AVG_DRTG - opp_oe) / LEAGUE_AVG_DRTG
    # Clamp to reasonable range
    reb_opp_adj = max(0.90, min(reb_opp_adj, 1.15))

    # Teammate absence rebound boost
...
def run_projections(date_str: str, generate_pdf: bool = False):
    ...
    print(f"\n{'='*70}")
    print(f"  NBA Player Props Projections — {date_str[:4]}-{date_str[4:6]}-{date_str[6:]}")
    print(f"{'='*70}\n")
    ...
    # Compute league averages from CSV
    if all_ratings:
    ...
    for g in games:
        ...
        print(f"       {g['short_name']:18s}  Spread: {sp:15s}  O/U: {ou}")
    print()

    # Fetch B2B status for each team
    ...
    # Fetch rosters + injuries for each game
    print("[4/7] Fetching rosters and injuries...")
    game_data = []  # list of {game, home_players, away_players, injuries}

    # Fetch all injuries in parallel
    injury_map = {}  # event_id -> {player_id: status}
    ...
    # Fetch prop lines for each game (multi-book via The Odds API)
    ...
    # Fallback: for any game the multi-book API didn't cover (rate-limited,
    # key exhausted, or no books posted yet), use ESPN's single-book
    # (DraftKings) props so the run still produces lines. ESPN-sourced props
    # carry no book attribution, so the Bk/#Bk columns stay blank for them.
    missing = [g for g in games if g["event_id"] not in prop_lines_map]
    if missing:
        if not prop_lines_map:
            print("       Odds API returned nothing — falling back to ESPN DraftKings props...")
        else:
            print(f"       {len(missing)} game(s) not on Odds API — ESPN DraftKings fallback...")
        with ThreadPoolExecutor(max_workers=6) as pool:
            futures = {
                pool.submit(fetch_prop_lines, g["event_id"]): g["event_id"]
                for g in missing
            }
            for future in as_completed(futures):
                eid = futures[future]
                prop_lines_map[eid] = future.result()
    ...
    gamelog_cache = {}  # player_id -> [game dicts]
    stats_cache = {}    # player_id -> computed stats dict
    ...
    for gd in game_data:
        ...
        # Identify injured players with their stats (for redistribution)
        def get_injured_teammates(roster, injuries_dict):
            """Get list of injured teammates with their stats."""
            injured = []
            for p in roster:
                pid = p["id"]
        ...
        for side, roster, team_name, team_key, opp_key, is_home in [
            ...
            for player in roster:
                ...
                # Use simulation medians as the primary projection
                ...
                # Merge prop lines + compute EV from the sim distribution.
                # Book fields use .get() so ESPN-fallback props (no book
                # attribution) merge cleanly with blank books / 0 count.
    ...
    # Sort by simulation median points descending
    ...
    for gname, players in games_seen.items():
        ...
        # Only show players with prop lines first, then others
        ...
        if with_props:
            ...
            for p in with_props:
                ...
                # Rebounds prop (use simulation median)
...
def generate_pdf_report(projections, games, game_data, date_str):
    ...
    pdf_path = OUTPUT_DIR / f"NBA_Props_Projections_{dt_file}.pdf"
    ...
    elements = []
    elements.append(Paragraph(f"NBA Player Props Projections \u2014 {dt_label}", title_style))
    ...
    summary_header_style = ParagraphStyle(
        ...
        spaceBefore=6, spaceAfter=6, textColor=HexColor("#16213e"),
    )
    elements.append(Paragraph("Top Props by EV%", summary_header_style))

    # Collect all props with a recommendation (positive EV)
    top_props = []
    for p in projections:
        pts_ev = p.get("pts_ev", {})
        reb_ev = p.get("reb_ev", {})
        if pts_ev.get("rec"):
            best_ev = pts_ev["ev_over"] if pts_ev["rec"] == "OVER" else pts_ev["ev_under"]
            top_props.append({
                "player": p["player"], "pos": p["position"], "team": p["team"],
                "game": p["game"], "prop": "Points", "line": p["pts_line"],
                "sim": p.get("sim_median_pts", p["expected_pts"]),
        ...
        if reb_ev.get("rec"):
            ...
            top_props.append({
                "player": p["player"], "pos": p["position"], "team": p["team"],
                "game": p["game"], "prop": "Rebounds", "line": p["reb_line"],
                "sim": p.get("sim_median_reb", p["expected_reb"]),
    ...
    if len(sum_data) > 1:
        sum_col_w = [0.3*inch, 1.4*inch, 0.3*inch, 1.0*inch, 0.65*inch,
                     0.45*inch, 0.4*inch, 0.45*inch, 0.5*inch, 0.55*inch]
        ...
        # Color-code EV% column and Rec column
    ...
    for game_idx, (gname, players) in enumerate(games_map.items()):
        # Each game on its own page (all games get page break since summary is first)
        ...
        # Table data — columns:
        # Player, Pos, Team, ExpMin, SimPts, PtLine, PtEdge, PtOvEV, PtUnEV, PtRec,
        #                            SimReb, RbLine, RbEdge, RbOvEV, RbUnEV, RbRec, Notes
        table_data = [
            ["Player", "Pos", "Tm", "ExpM",
             "SimPt", "PtLn", "Edge", "OvEV%", "UnEV%", "Rec",
             "SimRb", "RbLn", "Edge", "OvEV%", "UnEV%", "Rec",
             "Notes"],
        ]

        # Show players with prop lines first, sorted by sim median pts
        all_sorted = sorted(players, key=lambda x: (
            0 if x["pts_line"] is not None else 1,
            -x.get("sim_median_pts", x["expected_pts"])
        ))
        for p in all_sorted:
            # Skip players without prop lines and low projections
            sim_pts = p.get("sim_median_pts", p["expected_pts"])
            if p["pts_line"] is None and sim_pts < 5:
                continue
        ...
        col_w = [1.35*inch, 0.22*inch, 0.25*inch, 0.38*inch,
                 0.38*inch, 0.35*inch, 0.38*inch, 0.42*inch, 0.42*inch, 0.42*inch,
                 0.38*inch, 0.35*inch, 0.38*inch, 0.42*inch, 0.42*inch, 0.42*inch,
                 0.75*inch]
        ...
        style_cmds = [
            ...
            ("ALIGN", (0, 0), (0, -1), "LEFT"),
            # Vertical separator between points and rebounds sections
            ("LINEAFTER", (9, 0), (9, -1), 1.0, HexColor("#16213e")),
            ("LINEAFTER", (3, 0), (3, -1), 1.0, HexColor("#16213e")),
        ]

        # Color-code EV and recommendation cells
        green_strong = HexColor("#c8e6c9")  # EV > 5%
        green_light = HexColor("#e8f5e9")   # EV > 0%
        red_light = HexColor("#ffebee")     # EV < 0%

        for i, row in enumerate(table_data[1:], 1):
            # Points EV over (col 7), under (col 8), rec (col 9)
            for ev_col in (7, 8):
                try:
                    ...
                    pass
            # Rebounds EV over (col 13), under (col 14)
            for ev_col in (13, 14):
                try:
                    val = float(row[ev_col])
                    if val >= 5.0:
                    ...
                    pass
            # Rec columns (9 and 15) - bold green for OVER/UNDER
            for rec_col in (9, 15):
    ...
    book_label_style = ParagraphStyle(
        ...
        spaceBefore=6, spaceAfter=1, textColor=HexColor("#16213e"),
    ...
    if book_blocks:
        ...
        allbooks_header_style = ParagraphStyle(
            ...
            spaceBefore=4, spaceAfter=6, textColor=HexColor("#16213e"),
        )
...
def main():
    parser = argparse.ArgumentParser(description="NBA Player Props Projections")
    parser.add_argument(
