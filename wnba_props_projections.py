...
def project_player(
    ...
    for tm in injured_teammates:
        ...
        if (is_big and tm_is_big) or (is_guard and tm_is_guard) or (is_wing and tm_is_wing):
            if base_min >= 22:
                injury_min_boost += absent_mpg * 0.20
            elif base_min >= 12:
                injury_min_boost += absent_mpg * 0.12
        else:
            if base_min >= 24:
                injury_min_boost += absent_mpg * 0.06
...
def run_projections(date_str: str, generate_pdf: bool = False):
    ...
    print(f"\n{'='*70}")
    print(f"  WNBA Player Props Projections — {date_str[:4]}-{date_str[4:6]}-{date_str[6:]}")
    print(f"{'='*70}\n")
...
def generate_pdf_report(projections, games, game_data, date_str):
    ...
    pdf_path = OUTPUT_DIR / f"WNBA_Props_Projections_{dt_file}.pdf"
